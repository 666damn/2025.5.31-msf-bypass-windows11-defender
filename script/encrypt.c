...
void chacha20_encrypt(unsigned char *data, size_t len, const unsigned char key[32], const unsigned char nonce[12]) {
...
void rc4_encrypt(unsigned char *data, size_t len, const unsigned char *key, size_t keylen) {
    unsigned char state[256];
    rc4_ksa(state, key, keylen);
    rc4_prga(state, data, len);
}

void export_enc_shellcode_h(unsigned char *data, size_t len) {
    FILE *f = fopen("enc_shellcode.h", "w");
    if (!f) {
        perror("fopen");
        exit(1);
    }

    fprintf(f, "#ifndef ENC_SHELLCODE_H\n#define ENC_SHELLCODE_H\n\n");
    ...
    fprintf(f, "\n};\n");
    fprintf(f, "unsigned int shellcode_len = %zu;\n\n#endif\n", len);
...
int main(int argc, char **argv) {
    ...
    // 加密顺序：ChaCha20 -> XOR -> RC4
    chacha20_encrypt(buf, len, chacha_key, chacha_nonce);
    xor_encrypt(buf, len, xor_key);
    rc4_encrypt(buf, len, rc4_key, strlen((char*)rc4_key));

    export_enc_shellcode_h(buf, len);

    free(buf);
    printf("[+] enc_shellcode.h generated, length=%ld bytes\n", len);
